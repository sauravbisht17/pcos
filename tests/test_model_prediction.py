...
@pytest.fixture
def network_model(preprocessor):
...
def test_model_loading(network_model):
    assert network_model is not None

def test_model_prediction(network_model):
    # Ensure the test data has the same column names as the training data
    feature_names = network_model.preprocessor.named_steps['imputer'].feature_names_in_
    data.columns = feature_names

    processed_data = network_model.preprocessor.transform(data)
    prediction = network_model.predict(processed_data)
